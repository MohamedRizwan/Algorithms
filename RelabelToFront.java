...
public class RelabelToFront {
	...
	private int[] discharge(FlowNetwork fn, int target, int[] currents) {
		...
		while (fn.excess[target] > 0) {
			...
			if (neighbInd >= fn.neighbList[target].length) {
				// if we have, then we must relabel and reset our current
				// inspection to the front of the list
				relabel(fn, target);
				currents[target] = 0;
			} else {
				// otherwise find the id of the neighbor, check if it is a valid
				// push target and push, if not move to next neighbor
	...
	private void initialize() {
		...
		fn.excess[fn.src] = INF;// temporarily allow source to have infinite
								// excess to utilize push
